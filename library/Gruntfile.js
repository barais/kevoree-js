...
module.exports = function (grunt) {
    ...
    grunt.registerTask(
        'publish',
        "Runs 'npm publish' in every module folder",
        publishTask
    );
    grunt.registerTask(
        'unpublish',
        "Runs 'npm unpublish --force' in every module folder",
        unpublishTask
    );
    grunt.registerTask(
        'version',
    ...
    grunt.registerTask('publish-all', ['clean', 'install', 'publish']);
    grunt.registerTask('patch-publish-all', ['clean', 'install', 'version:patch', 'publish']);
    grunt.registerTask('minor-publish-all', ['clean', 'install', 'version:minor', 'publish']);
    grunt.registerTask('major-publish-all', ['clean', 'install', 'version:major', 'publish']);
};
...
var cleanTask = function cleanTask() {
    ...
    doInEachModule(done, function (modulePath, callback) {
        ...
        rimraf(nodeModulesPath, function (err) {
            ...
            // deletion succeed
            console.log("Clean node_modules/ for %s", modulePath);
...
/**
 * publish task - runs 'npm publish' in each module directory
 */
var publishTask = function publishTask() {
    var done = this.async();

    doInEachModule(done, function (modulePath, callback) {
        var exec = require('child_process').exec;
        exec('npm publish', {cwd: modulePath}, function(err, stdout, stderr) {
            console.log(stdout);
            callback(null);
        });
    });
...
/**
 * unpublish task - runs 'npm unpublish --force' in each module directory
 */
var unpublishTask = function unpublishTask() {
    var done = this.async();

    doInEachModule(done, function (modulePath, callback) {
        var exec = require('child_process').exec;
        exec('npm unpublish --force', {cwd: modulePath}, function(err, stdout, stderr) {
            console.log(stdout);
            callback(null);
        });
    });
...
var doInEachModule = function doInEachModule(doneCallback, command) {
    ...
    fs.readdir(__dirname, function (err, files) {
        ...
        async.parallel(moduleTasks, function (err, results) {
